...
function SideNav() {
  ...
  return (
    <div className="hidden md:flex w-[250px] bg-mediumGrey border-[#979797] border-r max-h-screen h-full fixed  md:flex-col">
      <div className="flex-1">
        ...
        <div className="flex flex-col gap-4">
          <h3 className="text-xs text-[#828FA3] uppercase px-[26px]">
            ALL BOARD ({boardLength})
          </h3>
          <BoardList boardNames={boardNames} selectedBoard={selectedBoard} />
        </div>
      </div>
      <div className="h-[48px] flex items-center justify-center w-full my-6">
        <div className="flex gap-6 items-center justify-center bg-veryDarkGrey mx-6 w-full py-3 rounded-lg">
...
export const BoardList = ({ boardNames, selectedBoard }: BoardListProps) => {
  return (
    <div className="flex flex-col w-full items-start">
      {boardNames.map((boardName, index) => (
        <div
          key={index}
          className={`flex px-[26px]  gap-2 py-[14px] items-center ${
            selectedBoard === boardName
              ? "bg-primaryPurple rounded-r-full w-[90%]"
              : ""
          }`}
        >
